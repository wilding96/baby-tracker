      ...
      <Card className="island-card bg-[#fffdf5]">
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-sm text-[#9f927d]">还没有留言，来写第一条吧。</p>
          ) : (
            <ul className="space-y-3">
              {messages.map((msg) => (
                <li key={msg.id} className="rounded-3xl border-2 border-[#e8dcc8] bg-[#fffdf5] p-3 shadow-sm">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-[#725d42]">{msg.nickname || "匿名用户"}</p>
                    <p className="text-xs text-[#9f927d]">{new Date(msg.created_at).toLocaleString("zh-CN")}</p>
                  </div>
                  <p className="mt-1 text-sm text-[#725d42] whitespace-pre-wrap break-words">{msg.content}</p>
                </li>
              ))}
            </ul>
          )}
