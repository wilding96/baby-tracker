...
import { 
  Droplets, Moon, ArrowLeft, Clock, Check, 
  Baby, Sun, Utensils
} from "lucide-react";
...
export default function RecordPage() {
  ...
  const handleSave = async () => {
    ...
    // 2. 构造 Payload
    // 使用 Record<string, unknown> 或者具体的接口来代替 object
    ...
    if (view === "feeding") {
      ...
      details = {
        duration_minutes: differenceInMinutes(new Date(endTime), new Date(startTime))
      };
    }
  ...
  if (view === "menu") {
    return (
      <main className="container mx-auto max-w-md p-6 min-h-screen flex flex-col justify-center gap-6">
        <h1 className="text-2xl font-bold text-center mb-4 text-gray-800">记录什么？</h1>
        ...
        <Button variant="ghost" className="mt-8 text-gray-400" onClick={() => router.back()}>
          取消
        </Button>
      </main>
    );
  ...
  if (view === "feeding") {
    return (
      <FormLayout title="记录喂奶" color="text-blue-900" onBack={() => setView("menu")}>
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="flex justify-between items-end">
                ...
                <div className="text-3xl font-bold text-blue-600">
                  {amount[0]} <span className="text-sm text-gray-400 font-normal">ml</span>
                </div>
              </div>
              <Slider value={amount} onValueChange={setAmount} max={300} step={10} className="py-4" />
        ...
        <SaveButton loading={loading} onClick={handleSave} color="bg-blue-600 hover:bg-blue-700" />
      </FormLayout>
    );
  }
  ...
  if (view === "sleep") {
    return (
      <FormLayout title="记录睡眠" color="text-purple-900" onBack={() => setView("menu")}>
        <div className="space-y-6">
            ...
            <div className="bg-purple-50 p-4 rounded-xl text-center">
                ...
                <p className="text-2xl font-bold text-purple-800">{getSleepDuration()}</p>
            </div>
        </div>
        <SaveButton loading={loading} onClick={handleSave} color="bg-purple-600 hover:bg-purple-700" />
      </FormLayout>
    );
  }
  ...
  if (view === "diaper") {
    return (
      <FormLayout title="记录换尿布" color="text-orange-900" onBack={() => setView("menu")}>
        ...
        <SaveButton loading={loading} onClick={handleSave} color="bg-orange-500 hover:bg-orange-600" />
      </FormLayout>
    );
  }
